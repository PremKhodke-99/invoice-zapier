...
const Invoices = () => {
    ...
    const triggerZapier = async (invoiceId) => {
        try {
            await axios.post(`${process.env.REACT_APP_API_URL}/api/invoices/trigger-zapier`, { invoiceId });
            setModalMessage('Reminder sent');
        } catch (error) {
            console.error('Error triggering Zapier:', error);
            setModalMessage('Failed to trigger reminder');
        } finally {
            setShowModal(true);
        }
    };
